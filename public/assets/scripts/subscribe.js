(() => {
  ...
  if (!form || !resultNode || !submitBtn || !emailInput || !repoInput || !apiKeyInput) {
    return;
  }
  ...
  // Same rules as `readCreateSubscriptionBody` / `parseRepoSlug` on the server.
  const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const REPO_RE = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/;

  /** @param {'success' | 'error' | 'info'} variant */
  const showResult = (variant, message) => {
  ...
  const defaultSubmitLabel = submitBtn.textContent.trim() || 'Subscribe';
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showResult('error', `Something went wrong (${response.status}). ${message}`);
        return;
      }
